...
import org.rob4001.iAuction.payment.*;
import org.rob4001.iAuction.payment.Method.MethodAccount;
...
public class iAuction extends JavaPlugin {
    ...
    public void setupSettings() {
        ...
        anti = Settings.getBoolean("enable-anti-snipe", false);
        antiint = Settings.getInt("anti-snipe-value", 5);
    }
    ...
    public void auctionInfo(Server server, Player player) {
        if (server != null) {
            ...
            broadcast((new StringBuilder(String.valueOf(infoPrimaryColor))).append("Starting Price: ").append(infoSecondaryColor).append(Method.format(auctionItemStarting)).toString());
        ...
        if (player != null)
            if (isAuction) {
                ...
                player.sendMessage((new StringBuilder(String.valueOf(tag))).append(infoPrimaryColor).append("Current bid: ").append(infoSecondaryColor).append(Method.format(currentBid)).toString());
        ...
        if ((permissionsEnabled || groupManagerEnabled) && Permissions.has(player, "auction.end") || player == auctionOwner || player.isOp()) {
            if (isAuction) {
                ...
                if (win) {
                    ...
                    auctionOwner.sendMessage((new StringBuilder(String.valueOf(tag))).append(auctionStatusColor).append("Your items have been sold for ").append(Method.format(currentBid)).append("!").toString());
                    Method.getAccount(winner.getName()).subtract(currentBid);
                    Method.getAccount(auctionOwner.getName()).add(currentBid);
        ...
        if (!permissionsEnabled && !groupManagerEnabled || Permissions.has(player, "auction.bid")) {
            if (msg.length == 2 || msg.length == 3) {
                if (player != auctionOwner) {
                    ...
                    MethodAccount acc = Method.getAccount(name);
                    int bid;
                    int sbid;
                    try {
                    ...
                    if (bid <= acc.balance() && sbid <= acc.balance()) {
                        if (isAuction) {
                            if (bid > currentBid) {
                                ...
                                if (bid > auctionItemBid) {
                                    ...
                                    auctionItemBid = sbid;
                                    winner = player;
                                    broadcast((new StringBuilder(String.valueOf(auctionStatusColor))).append("Bid raised to ").append(auctionTimeColor).append(Method.format(bid)).append(auctionStatusColor).append(" by ").append(auctionTimeColor).append(player.getDisplayName()).toString());
                                    if (anti) i += antiint;
                                } else {
                                    ...
                                    broadcast((new StringBuilder(String.valueOf(auctionStatusColor))).append("Bid raised! Currently stands at: ").append(auctionTimeColor).append(Method.format(bid + 1.0D)).toString());
                                    currentBid = bid + 1;
                                }
                        ...
                        warn(player, (new StringBuilder("Your current balance is: ")).append(Method.format(acc.balance())).toString());
                    }
                } else {
