...
public class Items {
    ...
    public static String name(int id, int data) {
    	...
    	if (iAuction.items.containsKey(Misc.string(id))) {
    		return Misc.camelToPhrase(iAuction.items.get(Misc.string(id)));
    	} else if (data >= 0 && iAuction.items.containsKey(longKey)) {
    		return Misc.camelToPhrase(iAuction.items.get(longKey));
    	}
    ...
    public static void setName(String id, String name) {
    	iAuction.items.put(id, name);
    	iAuction.Item.setString(id, name);
    }
...
*/
    public static int[] validate(String item) {
    	...
    	try {
    		...
    		String val = "";
    		for (String id : iAuction.items.keySet()) {
    			if(id.equalsIgnoreCase(item)){
    				val = iAuction.items.get(id);
    				//iAuction.log.info("Equals key: " + item + "=" + val);
    			} else if(iAuction.items.get(id).equalsIgnoreCase(item)) {
    				val = id;
    				//iAuction.log.info("Equals val: " + val + "=" + item);
    			}
    		}
    		//iAuction.log.info("Resultant name: " + (val.isEmpty()?"<empty>":val));
    		if (val.contains(",")) {
    ...
    public static int validateGrabType(String item) {
    	...
    	try {
    		...
    		for (String id : iAuction.items.keySet()) {
    			if (iAuction.items.get(id).equalsIgnoreCase(item)) {
    				if (id.contains(",")) {
